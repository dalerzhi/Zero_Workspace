...
cmd_speak() {
  ...
  if [[ "$backend" == "kokoro" ]]; then
    ...
    if [[ -z "$voice_id" && -z "$ref_audio" ]]; then
      # Only use default reference audio when NEITHER voice-id nor ref-audio is specified.
      # When voice-id is provided, use the built-in voice directly.
      ...
      echo "[noiz] Using built-in voice: $voice_id" >&2
    fi
