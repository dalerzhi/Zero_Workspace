...
- **终局规则（更高优先级）**：所有长时间运行的 CLI 子 agent / coding agent / 写作 agent，默认必须通过 watchdog 启动；不允许再裸跑 `nohup xxx &` 然后靠猜测等待。
- **默认入口**：标准任务优先用 `python3 scripts/run-agent-task.py --kind writing|research|coding ...`；复杂场景再直接用 `python3 scripts/agent-watchdog.py ...`。
...
- **设计原则**：不把“任务已提交”当作“agent 已启动”；不把“还在跑”当作“有进展”；不允许无限等待。详细说明见 `AGENT-WATCHDOG.md` 和 `AGENT-TASK-RUNNER.md`。
