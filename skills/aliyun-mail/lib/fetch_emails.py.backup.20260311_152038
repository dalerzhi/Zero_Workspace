...
def parse_credentials_ini(cred_file):
    """解析 INI 格式凭证文件"""
    import configparser
    ...
    for section in config.sections():
        ...
        for key in config[section]:
            account[key] = config[section][key]
        accounts.append(account)
    
    return accounts


def parse_credentials_old(cred_file):
    """解析旧版 Markdown 格式凭证文件"""
    accounts = []
    current_account = {}
...
def parse_credentials(cred_file):
    ...
    if first_line.startswith('[') and first_line.endswith(']'):
        # INI 格式
        return parse_credentials_ini(cred_file)
    else:
        # Markdown 格式（旧版）
        return parse_credentials_old(cred_file)
...
def load_feishu_credentials():
    ...
    home_dir = Path.home()
    cred_file = home_dir / '.openclaw' / '.feishu-credentials'
    ...
    # 使用 default 账户或环境变量指定的账户
    account_name = os.environ.get('FEISHU_ACCOUNT', 'default')
    
    if account_name not in config:
        account_name = 'default'
    
    return {
        'app_id': config.get(account_name, 'app_id', fallback=''),
        'app_secret': config.get(account_name, 'app_secret', fallback=''),
        'default_chat_id': config.get(account_name, 'default_chat_id', fallback='')
    }


def send_to_feishu(markdown_content, chat_id=None):
    """发送 Markdown 消息到飞书"""
    # 加载飞书凭证
    feishu_creds = load_feishu_credentials()
    app_id = feishu_creds['app_id']
    app_secret = feishu_creds['app_secret']
    
    # 使用传入的 chat_id 或默认值
    if not chat_id:
        chat_id = feishu_creds['default_chat_id']
    
    if not app_id or not app_secret:
        print("❌ 飞书凭证未配置，请创建 ~/.openclaw/.feishu-credentials")
        return False
    
    if not chat_id:
        print("❌ 飞书 chat_id 未指定，请使用 --chat-id 或配置 default_chat_id")
        return False
...
def get_user_email_from_env():
    ...
    # 尝试从凭证文件读取第一个账户
    home_dir = Path.home()
    cred_file = home_dir / '.openclaw' / '.email-credentials'
...
def is_user_recipient(email, user_email=None):
    """检查用户是否是收件人（而非仅抄送）"""
    if user_email is None:
        user_email = get_user_email_from_env()
    
    if not user_email:
        return True  # 无法确定时默认是收件人
    ...
    # 尝试中文字名（从环境变量或配置读取）
    import os
    user_name_cn = os.environ.get('EMAIL_USER_NAME', '')
    if user_name_cn:
        user_names.extend([user_name_cn, user_name_cn[0]])  # 全名和姓氏
    
    return any(name.lower() in to_field for name in user_names)
...
def main():
    ...
    # 优先使用新格式凭证文件
    cred_file = openclaw_dir / '.email-credentials'
    ...
    if not cred_file.exists():
        print(f"❌ 凭证文件不存在")
        print(f"请创建：~/.openclaw/.email-credentials")
        print("\n示例格式：")
        print("  [main]")
        print("  email = your-email@company.com")
        print("  imap_server = imap.qiye.aliyun.com")
        print("  imap_port = 993")
        print("  auth_code = your-auth-code")
        sys.exit(1)
